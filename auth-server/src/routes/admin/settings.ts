...
import type { SystemSettingsRepository } from "../../repositories/system-settings.js";
import type { AuditLogRepository } from "../../repositories/audit-logs.js";

type Options = { systemSettingsRepository: SystemSettingsRepository; auditLogRepository: AuditLogRepository };

export const adminSettingsRoutes: FastifyPluginAsync<Options> = async (fastify, options) => {
  ...
  fastify.get<{ Params: { key: string } }>(
    ...
    async (request) => {
      ...
      if (!setting) throw fastify.httpErrors.notFound("Setting not found");
      return setting;
    }
  );
  ...
  fastify.put<{ Params: { key: string }; Body: { value: unknown; description?: string } }>(
    ...
    async (request) => {
      ...
      const setting = await options.systemSettingsRepository.upsert(
        request.params.key,
        request.body.value,
        actor.userId,
  ...
  fastify.delete<{ Params: { key: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!deleted) throw fastify.httpErrors.notFound("Setting not found");
