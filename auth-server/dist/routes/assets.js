...
export const assetRoutes = async (fastify, options) => {
    fastify.get("/v1/assets", { preHandler: fastify.authenticate }, async (request) => {
     ...
     */
    fastify.get("/v1/assets/:id/thumbnail", { preHandler: fastify.authenticate }, async (request, reply) => {
        const { id } = request.params;
        const sizeParam = request.query.size ?? "medium";
        const size = VALID_SIZES.has(sizeParam) ? sizeParam : "medium";
        ...
        let cdnUrl;
        try {
            cdnUrl = await options.graphService.getThumbnailCdnUrl(msAccount, asset.driveId, asset.driveItemId, asset.storagePath, size);
        }
        catch (err) {
